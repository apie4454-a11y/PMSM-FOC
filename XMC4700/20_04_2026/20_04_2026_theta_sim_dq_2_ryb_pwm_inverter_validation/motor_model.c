...
void motor_step(MotorModel *m, float vd, float vq) {
    ...
    /* ========== Mechanical dynamics ========== */
    /* J*domega_mech/dt = T_e - B*omega_mech - Tf_friction - T_load */
    /* Coulomb friction opposes motion: sign(omega_mech) */
    float friction_torque = 0.0f;
    if (omega_mech > 0.0001f) {
        friction_torque = MOTOR_TF_COULOMB;      /* Friction opposes forward motion */
    } else if (omega_mech < -0.0001f) {
        friction_torque = -MOTOR_TF_COULOMB;     /* Friction opposes reverse motion */
    }
    /* else: friction_torque = 0 when omega near zero (static friction) */
    
    float domega_mech_dt = (T_e - (MOTOR_B * omega_mech) - friction_torque - m->T_load) / MOTOR_J;
