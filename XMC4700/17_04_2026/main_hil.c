...
/* External FOC function (from foc_algorithm_sil.c) */
/* 
 * Must implement:
 * void foc_step_update(float id, float iq, float motor_rpm, float omega_ele, 
 *                      float speed_ref, float max_flux,
 *                      float *vd_ref, float *vq_ref)
 * 
 * For now, we'll define a stub that you can replace with your C version
 */
extern void foc_algorithm_step(float id, float iq, float motor_rpm, float omega_ele,
                               float speed_ref, float max_flux,
                               float *vd_ref, float *vq_ref);
...
void hil_step_foc(HIL_State *hil) {
    ...
    /* Inverse Park: dq -> αβ */
    Clarke_Output alpha_beta = inverse_park_transform(hil->vd_ref, hil->vq_ref, hil->theta_ele);
    
    /* Inverse Clarke: αβ -> abc (with PWM saturation) */
    /* For simplicity, assume 3-phase balanced; compute duty cycles */
    /* Saturation will happen naturally based on Vdc */
    
    /* Simple mapping: vd,vq -> duty cycles */
    /* Assuming PWM runs at 20 kHz with 6000 counts per period */
    /* Center duty = 3000 (50%), range ±3000 */
    const float DUTY_CENTER = 3000.0f;
    const float DUTY_MAX_SWING = 3000.0f;
    
    /* Rough mapping (can be refined) */
    float duty_a_f = DUTY_CENTER + (hil->vd_ref / hil->inverter.vdc) * DUTY_MAX_SWING;
    float duty_b_f = DUTY_CENTER + (hil->vq_ref / hil->inverter.vdc) * DUTY_MAX_SWING;
    float duty_c_f = DUTY_CENTER - (hil->vd_ref + hil->vq_ref) / hil->inverter.vdc * DUTY_MAX_SWING;
    ...
    if (hil->foc_counter >= 10) {  /* 20 kHz / 10 = 2 kHz */
        ...
        /* Currently handled inside foc_algorithm_step */
    }
}
