...
void foc_init(FOCController *foc) {
    ...
    /* Speed decimation (CRITICAL: prevents 20kHz vs 2kHz mismatch) */
    foc->speed_decimator = 0;
    foc->iq_ref_prev = 0.0f;
    foc->te_ref_prev = 0.0f;
...
/*
 * Main FOC step (20 kHz call rate)
 * 
 * CRITICAL IMPLEMENTATION NOTE (17-04-2026):
 * This function is called at 20 kHz by the hardware interrupt.
 * The speed PI controller MUST run at 2 kHz (not 20 kHz), so we use
 * a decimation counter to skip 9 cycles and execute only every 10th call.
 * This matches the original design where Ts=0.0005s assumes 2 kHz execution.
 * Without decimation: 10x gain error on PI integrator -> oscillations (tested in SIL).
 * 
 * Cascade sequence:
 * 1. Speed PI: RPM error -> torque reference (DECIMATED: every 10 calls = 2 kHz)
 * 2. Torque->Current: Te_ref -> iq_ref (id_ref = 0 MTPA)
 * 3. Current PIs: id/iq errors -> vd/vq voltage references (runs every call)
...
void foc_step(FOCController *foc,
    ...
    /* ===== SPEED LOOP DECIMATION (2 kHz from 20 kHz) ===== */
    float te_ref;
    float iq_ref;
    
    foc->speed_decimator++;
    
    if (foc->speed_decimator >= 10) {
        /* Execute speed PI only every 10th call (2 kHz rate) */
        foc->speed_decimator = 0;
        ...
        te_ref = pi_step(&foc->speed_pi, rpm_error_rad_s);
        foc->speed_error = rpm_error;
        foc->te_ref = te_ref;
        
        /* Store for hold period */
        foc->te_ref_prev = te_ref;
        
        /* STEP 2: TORQUE -> CURRENT REFERENCE */
        iq_ref = te_ref / (1.5f * foc->pp * max_flux);
        foc->iq_ref_prev = iq_ref;
    } else {
        /* Hold previous values for 9 cycles */
        te_ref = foc->te_ref_prev;
        iq_ref = foc->iq_ref_prev;
        /* speed_error and te_ref outputs stay frozen during hold */
        /* This ensures integrator gains are correct at 2 kHz rate */
    }
